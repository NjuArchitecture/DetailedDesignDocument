...
#### 2.2.6. BeanFactory类
测试模块创建代理类实例的工厂，根据需要为不同的目标类动态产生其代理类并实例化。
...
![测试模块顺序图](assets/dbc/测试模块顺序图.png)
...
### 4.2 代理模式
系统需要在进入测试模式时能够在不破坏原有业务逻辑的基础上增加新的日志记录功能，而代理模式正好可以达到这个要求。代理模式通过为每个业务逻辑类添加新的代理，从而让client实际上拿到的引用并不是真正的目标对象，而只是一个假的proxy。这样client在调用业务方法时，系统就可以插入日志记录的增强过程，通过调用的转发让client的调用请求也得以正常的执行。

代理模式在类图中的体现如下：  
![代理模式的类图体现](assets/dbc/代理模式的体现.png)  
